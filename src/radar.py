...
from utils.utils import create_radar_frame, euclidean_distance, get_crops, save_all_frames_to_json
...
class RadarView:
    ...
    def _initialize_components(self) -> None:
        ...
        self.player_tracker = sv.ByteTrack(minimum_consecutive_frames=self.min_consecutive_frames)
    ...
    def _detect_pitch_keypoints(self, frame: np.ndarray) -> Tuple[sv.KeyPoints, ViewTransformer]:
        """
    ...
    def _detect_and_track_ball(self, frame: np.ndarray) -> sv.Detections:
        ...
        if len(ball_detections) == 0:
            if (self.last_ball_detections is not None and 
                    self.ball_missing_frames < self.ball_missing_threshold):
    ...
    def _annotate_possession_info(
        self,
        frame: np.ndarray,
        current_ball_possession_team: Optional[int]
    ) -> None:
        """
        ...
        if self.total_frames > 0:
            pos_a_percent = (self.possession_counts[TEAM_A_ID] / self.total_frames) * 100
            pos_b_percent = (self.possession_counts[TEAM_B_ID] / self.total_frames) * 100
    ...
    def _save_frame_data(
        ...
        frame_data = {
            ...
            'players': [
                {
                    ...
                    'team_id': int(team_id),
                    'position': list(pos),
                    'jersey_number': self.assigned_jersey_numbers[team_id].get(tracker_id, None)
                }
            ...
            'balls': [
                {'position': list(pos)}
                for pos in (transformed_ball_positions / 100.0)  # Scale to match player positions
            ]
        }
        
        self.all_frames.append(frame_data)
    
    def process_video(self, source_video_path: str, json_file_path: Optional[str] = None) -> Iterator[np.ndarray]:
        """
        ...
        for frame in frame_generator:
            ...
            players, players_team_id, player_labels = self._detect_and_track_players(frame)
            
            # Detect goalkeepers and referees
            result = self.player_model(frame, imgsz=1280, verbose=False)[0]
            detections = sv.Detections.from_ultralytics(result)
            detections = self.player_tracker.update_with_detections(detections)
            goalkeepers, referees, goalkeepers_team_id = self._detect_goalkeepers_and_referees(
            ...
            all_detections = sv.Detections.merge([players, goalkeepers, referees, ball_detections])
            ...
            all_labels = player_labels + [''] * (len(goalkeepers) + len(referees) + len(ball_detections))
